...
export default function Timetable() {
  ...
  useEffect(() => {
    if (activeTimetable) {
      setWeek(Math.max(1, Math.min(activeTimetable.totalWeeks, initialWeek)));
    }
  }, [initialWeek, activeTimetable?.totalWeeks]);
              ...
              return (
                <div key={i} className="text-center pb-1 flex flex-col items-center">
                  ...
                  <div className={`w-7 h-7 mt-0.5 flex items-center justify-center rounded-full ${isToday ? 'bg-primary text-white shadow-sm' : ''}`}>
                    <p className={`text-base font-black ${isToday ? '' : isWeekend ? 'text-rose-300' : 'text-slate-400'}`}>{d.getDate()}</p>
                  </div>
                </div>
              );
            })}
